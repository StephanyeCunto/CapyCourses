...
public class CursoBasicsValid {
    ...
    public void setupInitialStateBasics(TextField titleCourse, TextArea descritionCourse,
            ...
            Label descritionBasicsErrorLabel, Label categoryCourseErrorLabel, Label levelCourseErrorLabel) {
        loadValues(titleCourse, descritionCourse, categoryCourse, levelCourse, basicsTitleErrorLabel,
                descritionBasicsErrorLabel, categoryCourseErrorLabel, levelCourseErrorLabel);
        ...
        titleCourse.textProperty().addListener((obs, old, newText) -> {
            if (newText.length() >= MIN_TITLE_LENGTH) {
                updateErrorDisplay(titleCourse, basicsTitleErrorLabel, false, null);
            }
        });

        titleCourse.focusedProperty().addListener((obs, wasFocused, isNowFocused) -> {
            validationSupport.registerValidator(titleCourse, false,
        ...
        descritionCourse.textProperty().addListener((obs, old, newText) -> {
            if (newText.length() >= MIN_DESCRIPTION_LENGTH) {
                updateErrorDisplay(descritionCourse, descritionBasicsErrorLabel, false, null);
            }
        });

        descritionCourse.focusedProperty().addListener((obs, wasFocused, isNowFocused) -> {
            validationSupport.registerValidator(descritionCourse, false,
                    Validator.createPredicateValidator(value -> {
                        if (value instanceof String) {
                            String strValue = (String) value;
        ...
        categoryCourse.valueProperty().addListener((obs, old, newValue) -> {
            if (newValue != null && !newValue.isEmpty()) {
                updateErrorDisplay(categoryCourse, categoryCourseErrorLabel, false, null);
            }
        });

        categoryCourse.focusedProperty().addListener((obs, wasFocused, isNowFocused) -> {
            validationSupport.registerValidator(categoryCourse, false,
                    Validator.createPredicateValidator(value -> {
                        if (value instanceof String) {
                            String strValue = (String) value;
        ...
        levelCourse.valueProperty().addListener((obs, old, newValue) -> {
            if (newValue != null && !newValue.isEmpty()) {
                updateErrorDisplay(levelCourse, levelCourseErrorLabel, false, null);
            }
        });

        levelCourse.focusedProperty().addListener((obs, wasFocused, isNowFocused) -> {
            validationSupport.registerValidator(levelCourse, false,
                    Validator.createPredicateValidator(value -> {
                        if (value instanceof String) {
                            String strValue = (String) value;
                            return strValue != "null";
    ...
    private void loadValues(TextField titleCourse, TextArea descritionCourse, ComboBox<String> categoryCourse,
            ComboBox<String> levelCourse, Label basicsTitleErrorLabel, Label descritionBasicsErrorLabel,
            Label categoryCourseErrorLabel, Label levelCourseErrorLabel) {
    ...
    /*
     * private static final ValidationSupport validationSupport = new
     * ValidationSupport();
     ...
     * 
     * public void setupInitialState(TextField titleCourse, TextArea
     * descritionCourse,
     ...
     * }
     * 
     * private void updateErrorDisplay(Control field, Label errorLabel, boolean
     ...
     * boolean categoryValid = categoryCourse.getValue() != null &&
     * !categoryCourse.getValue().isEmpty();
     * boolean levelValid = levelCourse.getValue() != null &&
     * !levelCourse.getValue().isEmpty();
     ...
     * VBox modulesList = (VBox)
     * interestContainer.getScene().lookup("#modulesList");
     ...
     * titleContainer.lookup(".custom-text-field");
     * 
     * VBox durationContainer = (VBox) moduleContent.getChildren().get(1);
     * TextField durationField = (TextField)
     * durationContainer.lookup(".custom-text-field");
     * 
     * VBox detailsContainer = (VBox) moduleContent.getChildren().get(2);
     * TextArea detailsField = (TextArea)
     * detailsContainer.lookup(".custom-text-area");
     * 
     * boolean moduleFieldsValid = validateModuleFields(titleField, durationField,
     * detailsField);
     * 
     ...
     * 
     * showErrorNotification("Adicione pelo menos uma aula no módulo " +
     * moduleNumber.getText());
     * return false;
     * }
     * 
     ...
     * for (Node node : lessonsList.getChildren()) {
     * if (node instanceof VBox && ((VBox) node).getChildren().get(1) instanceof
     * VBox) {
     ...
     * 
     * } catch (Exception e) {
     * System.err.println("Erro ao configurar validação do módulo: " +
     * e.getMessage());
     * e.printStackTrace();
     * }
     * }
     ...
     * }
     * 
     * ErrorNotification notification = new ErrorNotification(notificationContainer,
     * message);
     * notification.show();
     * }
     * 
     ...
     * .count();
     * 
     * selectedCount.setText(count + " tag" + (count != 1 ? "s" : "") +
     * " selecionada" + (count != 1 ? "s" : ""));
     * }
     * 
     * public boolean validateModuleFields(TextField titleField, TextField
     * durationField, TextArea detailsField) {
     * if (validationStarted) {
     ...
     * TextField titleField = (TextField) titleContainer.getChildren().get(1);
     * TextField videoField = (TextField) videoContainer.getChildren().get(1);
     * TextArea descriptionField = (TextArea)
     * descriptionContainer.getChildren().get(1);
     * TextField durationField = (TextField) durationContainer.getChildren().get(1);
     * 
     * Label titleErrorLabel = (Label) titleContainer.getChildren().get(2);
     * Label videoErrorLabel = (Label) videoContainer.getChildren().get(2);
     * Label descriptionErrorLabel = (Label)
     * descriptionContainer.getChildren().get(2);
     * Label durationErrorLabel = (Label) durationContainer.getChildren().get(2);
     * 
     ...
     * 
     * } catch (Exception e) {
     * System.err.println("Erro ao configurar listeners da aula: " +
     * e.getMessage());
     * e.printStackTrace();
     * }
     * }
     ...
     * 
     * } catch (Exception e) {
     * System.out.println("Erro ao configurar validação do questionário: " +
     * e.getMessage());
     * e.printStackTrace();
     * }
     * }
