...
public class CourseReader {
    ...
    try (BufferedReader br =
      ...
      while ((line = br.readLine()) != null) {
        ...
        if (titleVerific.equals(title)) {
          ...
          boolean visibility =
              "public".equalsIgnoreCase(values[7]) || "true".equalsIgnoreCase(values[7]);

          CourseSettings courseSetting =
              new CourseSettings(
                  title, dateStart, durationTotal, isCertificate, isGradeMiniun, visibility);
  ...
  public List<Lessons> courseLessons(String titleCourseVerific, int moduleNumberVerific) {
    ...
    try (BufferedReader br =
        new BufferedReader(new FileReader("capycourses/src/main/resources/com/bd/bd_aulas.csv"))) {
      String line;
      while ((line = br.readLine()) != null) {
        String[] values = line.split(",");
        String titleCourse = values[0];
  ...
  public Course course(String titleVerific) {
    ...
    return courses.stream().filter(c -> c.getTitle().equals(titleVerific)).findFirst().orElse(null);
  }
