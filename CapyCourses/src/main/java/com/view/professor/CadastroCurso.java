...
public class CadastroCurso implements Initializable {
  ...
  @FXML private ImageView coursePreviewImage;
  @FXML private Button uploadButton;
  @FXML private VBox modulesList;
  @FXML private VBox date;
  @FXML private CheckBox dateEnd;

  @SuppressWarnings("rawtypes")
  @FXML
  private ComboBox ComboBoxVisibily;

  @FXML private TextField durationTotal;
  @FXML private CheckBox isCertificate;
  @FXML private CheckBox isGradeMiniun;
  ...
  @FXML Label durationTotalErrorLabel;
  @FXML Label comboBoxVisibilityErrorLabel;
  @FXML StackPane toggleButtonStackPane;
  ...
  public void initialize(URL location, ResourceBundle resources) {
    ...
    validatorBasic.setupInitialStateBasics(
        titleCourse,
        descritionCourse,
        categoryCourse,
        levelCourse,
    ...
    validatorSettings.setupInitialState(
        durationTotal, ComboBoxVisibily, durationTotalErrorLabel, comboBoxVisibilityErrorLabel);
  ...
  private double calculeLesson() {
    ...
    int totalFields =
        totalTitleFields
            + totalVideoFields
            + totalDetailsFields
            + totalMaterialsFields
            + totalDurationFields;
    int completedFields =
        totalValidatedTitle
            + totalValidatedVideo
            + totalValidatedDetails
            + totalValidatedMaterials
            + totalValidatedDuration;
    if (totalFields == 0) {
      totalFields = 5;
    }
    return (completedFields * 100.0) / totalFields;
  ...
  private double calculeQuestion() {
    ...
    if (calculeQuestionaire() != 0) {
      ...
      int totalValidated =
          validatorQuestoes.getValidatedQuestionFieldsCount()
  ...
  private void uploadImage() {
    ...
    fileChooser
        .getExtensionFilters()
        .add(new FileChooser.ExtensionFilter("Imagens", "*.png", "*.jpg", "*.jpeg", "*gif"));
  ...
  private void loadComboBoxLevel() {
    levelCourse.getItems().addAll("Iniciante", "Intermediario", "Avançado");
  }
  ...
  private void checkTitle() {
    titleCourse.setOnKeyPressed(
        event -> {
          titleCourse.setEditable(true);
        });

    titleCourse.setOnKeyReleased(
        event -> {
  ...
  public List<Map<String, Object>> saveModulesAndContent() {
    ...
    for (int i = 0; i < modulesList.getChildren().size(); i++) {
      ...
      Label moduleNumber =
          (Label) ((StackPane) moduleHeader.getChildren().get(0)).getChildren().get(0);
      ...
      for (Node contentNode : contentContainer.getChildren()) {
        ...
        if (contentBox.getStyleClass().contains("lesson")) {
          try {
            Map<String, Object> lessonData =
                saveLessonData(contentBox, cont, Integer.parseInt(moduleNumber.getText()));
            if (lessonData != null) {
  ...
  private Map<String, Object> saveLessonData(VBox lessonCard, int cont, int moduleNumber) {
    ...
    Label lessonNumber =
        (Label) ((StackPane) lessonHeader.getChildren().get(0)).getChildren().get(0);
  ...
  private Map<String, Object> savequestionaireData(VBox questionaireCard, int cont) {
    try {
      ...
      TextField titleField =
          (TextField) ((VBox) container.getChildren().get(0)).getChildren().get(1);
      TextArea descriptionArea =
          (TextArea) ((VBox) container.getChildren().get(2)).getChildren().get(1);
      TextField scoreField =
          (TextField) ((VBox) container.getChildren().get(4)).getChildren().get(1);
  ...
  private Map<String, Object> saveQuestionData(
      VBox questionCard, Map<String, Object> questionData, int k) {
    try {
      VBox questionContent = (VBox) questionCard.getChildren().get(1);

      TextArea questionText = (TextArea) questionContent.getChildren().get(2);
      TextField scoreField =
          (TextField) ((HBox) questionContent.getChildren().get(0)).getChildren().get(1);
  ...
  private void processOptions(
      VBox optionsContainer, List<Map<String, Object>> responses, boolean isSingleChoice) {
    for (Node node : optionsContainer.getChildren()) {
      if (node instanceof HBox) {
        HBox optionBox = (HBox) node;
    ...
    moduleHeader.setOnMouseClicked(
        event -> {
          ...
          moduleHeader.setStyle(
              isCurrentlyVisible ? "-fx-border-width: 1px;" : "-fx-border-width: 1px 1px 0 1px;");
        });
  ...
  private VBox createModuleContent(int moduleNumber) {
    ...
    Label moduleTitleLabelError =
        new Label("Por favor, insira um título válido, entre 5 e 100 caracteres.");
    ...
    detailsContainer.getChildren().addAll(createNumberField("Duração (horas) *", "Ex: 2.5", true));
    ...
    Label moduleDescripitonLabelError =
        new Label("Por favor, insira uma descrição válida, com pelo menos 10 caracteres");
  ...
  private void addNewquestionaire(VBox lessonsList, int moduleNumber) {
    ...
    addDiscursiveButton.setOnAction(
        e -> addNewQuestion(questionsContainer, lessonsList, "DISCURSIVE", moduleNumber));
    ...
    addSingleChoiceButton.setOnAction(
        e -> addNewQuestion(questionsContainer, lessonsList, "SINGLE_CHOICE", moduleNumber));
    ...
    addMultipleChoiceButton.setOnAction(
        e -> addNewQuestion(questionsContainer, lessonsList, "MULTIPLE_CHOICE", moduleNumber));
    ...
    contextMenu
        .getChildren()
        .addAll(addDiscursiveButton, addSingleChoiceButton, addMultipleChoiceButton);
  ...
  private void addNewLesson(VBox lessonsList, int moduleNumber) {
    ...
    removeLessonButton.setOnAction(
        e -> {
  ...
  private VBox createLessonContent(int lessonNumber) {
    ...
    Label lessonDetailsLabelError =
        new Label("Por favor, insira detalhes válidos, com no mínimo 10 caracteres");
  ...
  private void addNewQuestion(
      VBox questionsContainer, VBox lessonsList, String type, int moduleNumber) {
    ...
    Label questionErrorLabel =
        new Label("Por favor, insira uma pergunta válida, com pelo menos 10 caracteres.");
    ...
    questionContent
        .getChildren()
        .addAll(scoreHbox, scoreErrorLabel, questionText, questionErrorLabel);
  ...
  private void addDiscursiveFields(VBox container) {
    ...
    Label evaluationCriteriaErrorLabel =
        new Label(
  ...
  private void addSingleChoiceFields(VBox container, int moduleNumber) {
    ...
    addOptionButton.setOnAction(
        e -> addChoiceOption(optionsContainer, container, toggleGroup, true, moduleNumber));
    ...
    container
        .getChildren()
        .addAll(optionLabel, optionsContainer, addOptionButton, optionErrorLabel);
  ...
  private void addMultipleChoiceFields(VBox container, int moduleNumber) {
    ...
    addOptionButton.getStyleClass().add("outline-button");
    addOptionButton.setOnAction(
        e -> addChoiceOption(optionsContainer, container, null, false, moduleNumber));
  ...
  private void addChoiceOption(
    ...
    Label optionErrorLabel =
        new Label("Por favor, insira uma opção válida, com pelo menos 1 caractere.");
    optionErrorLabel.getStyleClass().add("error-label");
    optionErrorLabel.setVisible(false);
    optionErrorLabel.setManaged(false);
  ...
  private Node findNodeById(VBox container, String id) {
    return container.lookupAll("#" + id).stream().findFirst().orElse(null);
  }
