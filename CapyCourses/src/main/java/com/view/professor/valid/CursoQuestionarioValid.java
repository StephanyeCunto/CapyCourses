...
public class CursoQuestionarioValid {
    ...
    for (Node questionNode : questionsContainer.getChildren()) {
      ...
      if (((VBox) questionContent.getChildren().get(2)).getChildren().get(1) instanceof TextArea) {
        ...
        TextField titleField =
            (TextField) (((VBox) questionContent.getChildren().get(0)).getChildren().get(1));
        Label titleErrorLabel = (Label) questionContent.getChildren().get(1);
        TextArea questionTextArea =
            (TextArea) ((VBox) questionContent.getChildren().get(2)).getChildren().get(1);
        Label questionErrorLabel = (Label) questionContent.getChildren().get(3);
        TextField scoreField =
            (TextField) ((VBox) questionContent.getChildren().get(4)).getChildren().get(1);
        ...
        questionTextFieldsMap
            .computeIfAbsent(moduleNumber, k -> new ArrayList<>())
            .add(questionTextArea);
        questionErrorLabelsMap
            .computeIfAbsent(moduleNumber, k -> new ArrayList<>())
            .add(questionErrorLabel);
  ...
  private void setupValidationListeners(int moduleNumber) {
    ...
    for (int i = 0; i < questionTextFields.size(); i++) {
      setupQuestionTextValidation(
          questionTextFields.get(i), questionErrorLabels.get(i), validationSupport);
  ...
  private void setupQuestionTextValidation(
      TextArea questionField, Label errorLabel, ValidationSupport validationSupport) {
    questionField
        .textProperty()
        .addListener(
            (obs, old, newText) -> {
                      ...
                      value -> {
                        if (value instanceof String) {
                          ...
                          return !strValue.trim().isEmpty()
                              && strValue.length() >= MIN_QUESTION_LENGTH;
                        }
                        return false;
                      },
  ...
  private void setupScoreValidation(
      TextField scoreField, Label errorLabel, ValidationSupport validationSupport) {
    scoreField
        .textProperty()
        .addListener(
            (obs, old, newText) -> {
  ...
  private void setupTitleValidation(
      TextField titleField, Label errorLabel, ValidationSupport validationSupport) {
    titleField
        .textProperty()
        .addListener(
            (obs, old, newText) -> {
  ...
  public boolean validateFields(CheckBox isGradeMiniun) {
    ...
    for (Integer moduleNumber : titleFieldsMap.keySet()) {
      ...
      for (int i = 0; i < titleFields.size(); i++) {
        ...
        if (scoreFields.get(i).getText().isEmpty()
            || !scoreFields.get(i).getText().matches("\\d*\\.?\\d*")) {
          updateErrorDisplay(
              scoreFields.get(i),
        ...
        for (int j = 0; j < questionCard.getChildren().size(); j++) {
          if (j == 1) {
            ...
            if (questionsContent.getChildren().size() == 0) {
              ErrorNotification errorNotification =
                  new ErrorNotification(parentContainer, "Adicione uma questão ao questionário");
    ...
    if (isGradeMiniun.isSelected()) {
      if (getTitleFieldsCount() < 1) {
        ErrorNotification errorNotification =
            new ErrorNotification(parentContainer, "Adicione um questionário");
  ...
  public int getValidatedTitleFieldsCount() {
    return (int)
        titleFieldsMap.values().stream()
            .flatMap(List::stream)
            .filter(
                field ->
                    field.getText().length() >= MIN_TITLE_LENGTH && field.getText().length() <= 100)
            .count();
  }

  public int getValidatedQuestionTextFieldsCount() {
    return (int)
        questionTextFieldsMap.values().stream()
            .flatMap(List::stream)
