package com.view.elements.Calendario;
...
public class Calendario extends Region {
  ...
  public HBox getDateInputField() {
    ...
    dateInputField.setStyle(
        dateInputField.getStyle()
    ...
    calendarButton.setOnMouseEntered(
        e ->
            calendarButton.setStyle(
                calendarButton.getStyle() + "-fx-background-color: " + hoverColor + ";"));
  ...
  private Button createCalendarButton() {
    ...
    button.setStyle(
        "-fx-background-color: transparent;"
            ...
            + "-fx-padding: 8 12;");
    button.setOnMouseEntered(
        e -> button.setStyle(button.getStyle() + "-fx-text-fill: " + accentColor + ";"));
    button.setOnMouseExited(
        e ->
            button.setStyle(
                button
                    .getStyle()
                    .replace("-fx-text-fill: " + accentColor + ";", "-fx-text-fill: white;")));
  ...
  private HBox createHeader() {
    ...
    monthLabel =
        new Label(selectedDate.getMonth().getDisplayName(TextStyle.FULL, Locale.getDefault()));
  ...
  private Button createSelectionButton(String text, Runnable action) {
    ...
    button.setOnMouseEntered(
        e -> button.setStyle(button.getStyle() + "-fx-background-color: " + hoverColor + ";"));
    button.setOnMouseExited(
        e ->
            button.setStyle(
                button.getStyle().replace("-fx-background-color: " + hoverColor + ";", "")));
    ...
    for (Popup popup : activePopups) {
      ...
      if (minDate != null) {
        prevButton.setDisable(
            firstOfMonth.isBefore(minDate.withDayOfMonth(1))
  ...
  private HBox createYearNavigationHeader() {
    ...
    yearRangeLabel.setStyle(
        String.format(
            "-fx-font-weight: bold;" + "-fx-font-size: 12px;" + "-fx-text-fill: %s;", textColor));

    prevYear.setOnAction(
        e -> {
          yearGridStartYear -= 12;
          updateYearPopup();
        });

    nextYear.setOnAction(
        e -> {
          yearGridStartYear += 12;
          updateYearPopup();
        });
