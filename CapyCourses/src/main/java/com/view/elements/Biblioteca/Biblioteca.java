...
public class Biblioteca {
  ...
  private static void loadList(String status) {
    ...
    if (status.equals("todos")) {
      ...
      bibliotecaList =
          biblioteca.loadBiblioteca().stream()
  ...
  private static VBox createBibliotecaBox(BibliotecaDTO bibliotecaDTO, String status) {
    ...
    Label categoryLabel =
        createStyledLabel(bibliotecaDTO.getCategory().toUpperCase(), "Franklin Gothic Medium", 12);
    ...
    Label authorLabel =
        createStyledLabel("Por " + bibliotecaDTO.getAuthor(), "Franklin Gothic Medium", 14);
    authorLabel.getStyleClass().add("author");

    Label descriptionLabel =
        createStyledLabel(bibliotecaDTO.getDescription(), "Franklin Gothic Medium", 16);
    ...
    button.setOnAction(
        e -> {
          BibliotecaModal modal =
              new BibliotecaModal(
                  getDefaultWindow(),
                  "/Users/stephanye/Downloads/teste/python_para_iniciantes_ste_cunto_certificado.pdf",
                  bibliotecaDTO);
        });
  ...
  private static void reorganizeGrid(
      int numColumns, List<BibliotecaDTO> bibliotecaList, String status) {
    bibliotecaGrid.getChildren().clear();
    int i = 0;
    for (BibliotecaDTO bibliotecaDTO : bibliotecaList) {
      VBox bibliotecaBox = createBibliotecaBox(bibliotecaDTO, status);
